...
for k = 1:10
    ...
    %% The previous results are not used and all tasks are assigned together
    ...
    if flag == 1
        while abs(right-left) > 2
            ...
            softTaskSetTemp = softTaskSet0(1:middle, :);
            lambdaSoftTemp = lambdaSoft0(:, 1:middle);
            
            taskSetTemp = [hardTaskSet0; softTaskSetTemp];
            lambdaTemp = [lambdaHard0, lambdaSoftTemp];
        ...
        taskN(k) = hardTaskCount + softTaskCount - size(UnassignedTasks,1);
    else
        taskN(k) = 0;
    end
    
    t2 = clock;
    runtime(k) = etime(t2,t1);
    
end
% save('C:\Users\BUCT\Desktop\新建文件夹\1\数据\runtime','runtime')
% save('C:\Users\BUCT\Desktop\新建文件夹\1\数据\taskN','taskN')
% disp('fininsh!!!!!!!!!!')
